...
// Package jobs manages background jobs that the rcserver is running.
package jobs
...
func (jobs *Jobs) IDs() []string {
	...
	jbs := make([]string, 0, len(jobs.jobs))
...
func (jobs *Jobs) Get(id string) *Job {
	jobs.mu.RLock()
	defer jobs.mu.RUnlock()
	return jobs.jobs[id]
}
...
func (jobs *Jobs) NewSyncJob(ctx context.Context, in rc.Params) (*Job, context.Context) {
	...
	ctxG := accounting.WithStatsGroup(ctx, id.String())
...
func StartAsyncJob(fn rc.Func, in rc.Params) (rc.Params, error) {
	...
	out := make(rc.Params)
	out["jobid"] = job.ID.String()
	return out, nil
}

// ExecuteJob executes new job synchronously and returns a Param suitable for
// output.
func ExecuteJob(ctx context.Context, fn rc.Func, in rc.Params) (rc.Params, string, error) {
	job, ctx := running.NewSyncJob(ctx, in)
	job.run(ctx, fn, in)
	return job.Output, job.ID.String(), job.realErr
}
