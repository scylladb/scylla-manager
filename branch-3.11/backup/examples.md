...
> * [Schedule a backup for a specific DC, keyspace, or table](#schedule-a-backup-for-a-specific-dc-keyspace-or-table)
> * [Create an ad-hoc backup](#create-an-ad-hoc-backup)
> * [Create backup for archiving keyspace](#create-backup-for-archiving-keyspace)
> * [Perform a dry run of a backup](#perform-a-dry-run-of-a-backup)
> * [Monitor progress of the backup task](#monitor-progress-of-the-backup-task)
...
All parameters can be found in the [sctool backup](https://manager.docs.scylladb.com/branch-3.11/sctool/backup.md#sctool-backup) command reference.
If you want to check if all of your nodes can connect to the backup storage location see [Perform a dry run of a Backup](#dry-run).
Following examples will use Amazon S3 as a storage provider.

**Prerequisites**

1. Backup locations (buckets) created and configured.
...
* `-c` - the [name](https://manager.docs.scylladb.com/branch-3.11/sctool/cluster.md#cluster-add) you used when you created the cluster
...
This ID can be used to query the status of the backup task, to defer the task to another time, or to cancel the task See [Managing Tasks](https://manager.docs.scylladb.com/branch-3.11/sctool/task.md#task-commands).
...
Additional parameters can be used. Refer to [backup parameters](https://manager.docs.scylladb.com/branch-3.11/sctool/backup.md#sctool-backup).
...
Progress of the backup task can be monitored by using [sctool progress](https://manager.docs.scylladb.com/branch-3.11/sctool/progress.md#task-progress) command and providing UUID of the backup task.
