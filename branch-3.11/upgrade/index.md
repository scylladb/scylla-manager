...
#### NOTE
See [Compatibility Matrix](https://manager.docs.scylladb.com/branch-3.11/compatibility-matrix.md) for information about compatibility between ScyllaDB Manager and ScyllaDB versions.
...
1. **On the Manager server**, update the Manager repo file.
   ...
   Ubuntu
   ```console
   sudo wget -O /etc/apt/sources.list.d/scylla-manager.list https://downloads.scylladb.com/deb/ubuntu/scylladb-manager-3.11.list
   ```
...
#### NOTE
If you plan to use native backup after the upgrade, we recommend [configuring](https://manager.docs.scylladb.com/branch-3.11/backup/native-backup.md#configure-native-backup-in-scylla) it at this point.
