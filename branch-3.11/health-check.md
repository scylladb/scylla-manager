...
the [sctool tasks](https://manager.docs.scylladb.com/branch-3.11/sctool/task.md#task-list) command.
...
There are two types of CQL health check [Credentials agnostic health check](#credentials-agnostic-health-check) and [CQL query health check](#cql-query-health-check).

The results are available using the [sctool status](https://manager.docs.scylladb.com/branch-3.11/sctool/status.md#id1) command.
...
* UNAUTHORISED - Missing or Incorrect [Authentication Token](https://manager.docs.scylladb.com/branch-3.11/install-scylla-manager-agent.md#configure-auth-token) was used
* TIMEOUT - Timeout
...
<a id="credentials-agnostic-health-check"></a>

## Credentials agnostic health check

ScyllaDB Manager does not require database credentials to work.
CQL health check is based on sending [CQL OPTIONS frame](https://github.com/apache/cassandra/blob/trunk/doc/native_protocol_v4.spec#L302) and does not start a CQL session.
...
You may specify CQL `username` and `password` flags when adding cluster to ScyllaDB Manager using [sctool cluster add](https://manager.docs.scylladb.com/branch-3.11/sctool/cluster.md#cluster-add) command.
It’s also possible to add or change that using [sctool cluster update](https://manager.docs.scylladb.com/branch-3.11/sctool/cluster.md#cluster-update) command.
...
<a id="scylla-alternator-health-check"></a>
...
Please check the ScyllaDB Manager [ScyllaDB Manager Config](https://manager.docs.scylladb.com/branch-3.11/config/scylla-manager-config.md#configuration-file) to adjust timeouts for your cluster.
