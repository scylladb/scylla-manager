...
* All the Agents have the same [authentication token](https://manager.docs.scylladb.com/branch-3.11/install-scylla-manager-agent.md#configure-auth-token) configured.
...
1. From the ScyllaDB Manager Server, provide the IP address of one of the nodes, the generated auth token, and a custom name.
   ...
   * `--auth-token` is the [authentication token](https://manager.docs.scylladb.com/branch-3.11/install-scylla-manager-agent.md#configure-auth-token) you generated.
   ...
   To use a different repair schedule, see [Schedule a Repair](https://manager.docs.scylladb.com/branch-3.11/repair/examples.md#schedule-a-repair).

   ScyllaDB manager requires CQL credentials to the cluster with `--username` and `--password` flags.
   This enables [CQL query based health check](https://manager.docs.scylladb.com/branch-3.11/health-check.md#cql-query-health-check) compared to [credentials agnostic health check](https://manager.docs.scylladb.com/branch-3.11/health-check.md#credentials-agnostic-health-check) if you do not specify the credentials.
...
2. Verify the cluster you added has a registered repair task by running the `sctool tasks` command.
   ...
   * Repair - an automated repair task, starting at midnight tonight, repeating every seven days at midnight.
   ...
   #### NOTE
   If you want to change the schedule for the repair, use the [repair update sctool](https://manager.docs.scylladb.com/branch-3.11/sctool/repair.md#reschedule-a-repair) command.
