...
func TestClusterMoveHostToKnownHostsBefore016Integration(t *testing.T) {
	...
	cb := migrationCallback("016-cluster_drop_host.cql", migrate.BeforeMigration)
	registerMigrationCallback("016-cluster_drop_host.cql", migrate.BeforeMigration, func(ctx context.Context, session *gocql.Session, logger log.Logger) error {
