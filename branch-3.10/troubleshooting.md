...
You can also pass the `--port` flag to [sctool cluster add](https://manager.docs.scylladb.com/branch-3.10/sctool/cluster.md#cluster-add) command when registering a new cluster.
...
1. [Add ScyllaDB Manager Agent](https://manager.docs.scylladb.com/branch-3.10/install-scylla-manager-agent.md#install-agent) to the new node. Use the **same** authentication token as you did for the other nodes in this cluster. Do not generate a new token.
2. Confirm the node / datacenter was added by checking its [sctool status](https://manager.docs.scylladb.com/branch-3.10/sctool/status.md#id1). From the node running ScyllaDB Manager server run the `sctool status` command, using the name of the managed cluster.
   ```none
