...
To restore the content of the tables (rows), use the [sctool restore](https://manager.docs.scylladb.com/branch-3.10/sctool/restore.md#sctool-restore) command with the `--restore-tables` flag.
<br/>
...
* ScyllaDB Manager requires CQL credentials ([sctool cluster update –username –password](cluster-update)) with:
...
* Restoring the content of the tables assumes that the correct schema (identical as in the backup) is already present in the destination cluster.
  ...
  > it should be [restored](https://manager.docs.scylladb.com/branch-3.10/restore/restore-schema.md) first.
...
> * [Repair](https://manager.docs.scylladb.com/branch-3.10/sctool/repair.md#sctool-repair) restored tables
> * Reset all restored tables `tombstone_gc` to its original mode
> * Recreate restored views

Information about original `tombstone_gc` mode, views definitions and repair progress is included in [sctool progress –details](https://manager.docs.scylladb.com/branch-3.10/sctool/progress.md#task-progress).

<a id="restore"></a>

## 1-1 restore

ScyllaDB Manager supports much faster restore of tables content using the [sctool restore 1-1-restore](https://manager.docs.scylladb.com/branch-3.10/sctool/restore.md#sctool-1-1-restore) command.
...
Please refer to the [sctool restore 1-1-restore](https://manager.docs.scylladb.com/branch-3.10/sctool/restore.md#sctool-1-1-restore) documentation for more details.
