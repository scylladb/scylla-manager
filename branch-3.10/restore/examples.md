...
> * [Schedule restore](#schedule-restore)
> * [Complete the restore procedure - restore both the schema and the content of the tables](#complete-the-restore-procedure-restore-both-the-schema-and-the-content-of-the-tables)
> * [Perform a dry run of a restore](#perform-a-dry-run-of-a-restore)

## Prerequisites

1. Backup locations (buckets) with backed-up data created by [backup](https://manager.docs.scylladb.com/branch-3.10/sctool/backup.md#sctool-backup) task.
2. Snapshot tag of backup that you want to restore.

To get a list of all available snapshot tags, use [sctool backup list](https://manager.docs.scylladb.com/branch-3.10/sctool/backup.md#backup-list) command.
...
* `-c` - the [name or ID](https://manager.docs.scylladb.com/branch-3.10/sctool/cluster.md#cluster-add) of the restore destination cluster
...
1. Ensure that all prerequisites of restoring schema are met.

   The easiest way to achieve that is to perform restore schema on a new empty cluster.
2. Restore schema using [sctool restore](https://manager.docs.scylladb.com/branch-3.10/sctool/restore.md#sctool-restore).
   ```none
   sctool restore -c prod-cluster -L s3:test-bucket -T snapshot_tag --restore-schema
   restore/3208ff15-6e8f-48b2-875c-d3c73f545410
   ```
3. Ensure that restoring schema has finished using [sctool progress](https://manager.docs.scylladb.com/branch-3.10/sctool/progress.md#task-progress).
   ```none
   ...
   ╰───────────────┴─────────────┴──────────┴──────────┴────────────┴──────────────┴────────╯
   ```
4. Perform restore schema follow-up action (only for ScyllaDB **5.4**/**2024.1** or older) - [rolling restart](https://docs.scylladb.com/manual/stable/operating-scylla/procedures/config-change/rolling-restart.html).
5. Ensure that all prerequisites of restoring the content of the tables are met.

   The easiest way to achieve that is to restore schema with [sctool restore](https://manager.docs.scylladb.com/branch-3.10/restore/restore-schema.md) (so that’s what has already been done in this example).
   ...
   that those tables are [truncated](https://docs.scylladb.com/manual/stable/cql/ddl.html#truncate-statement).
6. Restore the content of the tables using [sctool restore](https://manager.docs.scylladb.com/branch-3.10/sctool/restore.md#sctool-restore).
   ```none
   ...
   ```
7. Ensure that restoring the content of the tables has finished using [sctool progress](https://manager.docs.scylladb.com/branch-3.10/sctool/progress.md#task-progress).
   ```none
