...
type RunProgress struct {
	...
	// StartedAt is set to the scylla task timestamp
	// with SM side fallback if it's not available.
	StartedAt time.Time
	// CompletedAt is set to the scylla task timestamp
	// with SM side fallback if it's not available.
	// CompletedAt is set only when scylla task reached terminal state (done or failed).
