...
func skipImpossibleSchemaTest(t *testing.T, client *scyllaclient.Client) {
	t.Helper()
	restoreSchemaFromSSTableSupport := IsRestoreSchemaFromSSTablesSupported(context.Background(), client)
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if target.RestoreSchema {
				skipImpossibleSchemaTest(t, h.srcCluster.Client)
			}
...
func TestRestoreResumeIntegration(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			backupTarget, err := h.srcBackupSvc.GetTarget(context.Background(), h.srcCluster.ClusterID, backupProps)
			if err != nil {
				t.Fatal(err)
			}
			...
			err = h.srcBackupSvc.Backup(context.Background(), h.srcCluster.ClusterID, h.srcCluster.TaskID, h.srcCluster.RunID, backupTarget)
			if err != nil {
				t.Fatal(err)
			}
			backupPr, err := h.srcBackupSvc.GetProgress(context.Background(), h.srcCluster.ClusterID, h.srcCluster.TaskID, h.srcCluster.RunID)
			if err != nil {
				t.Fatal(err)
			}
			...
			h.dstCluster.RunID = uuid.NewTime()
			err = h.dstRestoreSvc.Restore(context.Background(), h.dstCluster.ClusterID, h.dstCluster.TaskID, h.dstCluster.RunID, rawProps)
			if err != nil {
