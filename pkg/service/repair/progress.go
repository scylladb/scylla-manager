...
type ProgressManager interface {
	...
	// GetDoneRanges returns ranges already successfully repaired in the previous runs.
	GetDoneRanges(keyspace, table string) []scyllaclient.TokenRange
...
type dbProgressManager struct {
	...
	tableRanges    map[string]int   // Maps keyspace to its range count
...
func (pm *dbProgressManager) initProgress(plan *plan) error {
	pm.progress = make(map[hkt]*RunProgress)
		...
		for _, tp := range kp.Tables {
			for _, h := range plan.Hosts {
				...
				pm.progress[pk] = &RunProgress{
					...
					Table:       pk.Table,
					Size:        plan.HostSize[pk],
					TokenRanges: int64(plan.HostRanges[pk]),
				}
			}
		}
	}
...
func (pm *dbProgressManager) GetDoneRanges(keyspace, table string) []scyllaclient.TokenRange {
	sk := stateKey{
		keyspace: keyspace,
		table:    table,
	}
