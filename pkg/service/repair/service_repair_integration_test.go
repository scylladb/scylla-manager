...
func TestServiceRepairIntegration(t *testing.T) {
	...
	t.Run("repair colocated table", func(t *testing.T) {
		...
		Print("Then: repair of whole keyspace should succeed")
		h.runRepair(ctx, map[string]any{
			"keyspace": []string{ks},
		})
		h.assertDone(time.Minute)

		Print("Then: repair of just random and LWT table should fail")
		h.runRepair(ctx, map[string]any{
			"keyspace": []string{ks, "!" + ks + "." + baseTable},
		})
		h.assertError(time.Minute)

		Print("Then: repair of just base and LWT table should succeed")
		h.runRepair(ctx, map[string]any{
