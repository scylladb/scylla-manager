...
func TestTabletRepairIntegration(t *testing.T) {
	...
	kss, err := h.Client.FilteredKeyspaces(t.Context(), scyllaclient.KeyspaceTypeNonLocal, scyllaclient.ReplicationTablet)
	if err != nil {
		t.Fatal(err)
	}
	for _, ks := range kss {
		tabs, err := h.Client.Tables(t.Context(), ks)
		if err != nil {
			t.Fatal(err)
		}
	...
	t.Run("Smoke", func(t *testing.T) {
		testCtx, testCancel := context.WithTimeout(t.Context(), 5*time.Minute)
		defer testCancel()
		smokeTest(t, testCtx)
	})
