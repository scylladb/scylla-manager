...
// snapshotTagRetainUntil computes the expected retainUntil time from a snapshot tag.
func snapshotTagRetainUntil(t *testing.T, snapshotTag string, retentionDays int) time.Time {
	t.Helper()

	tagTime, err := backupspec.SnapshotTagTime(snapshotTag)
	if err != nil {
		t.Fatalf("parse snapshot tag %q: %v", snapshotTag, err)
	}
	return tagTime.Add(time.Duration(retentionDays) * 24 * time.Hour)
}
...
func TestBackupResumeOnRetentionLockStageIntegration(t *testing.T) {
	...
	go func() {
		done <- h.service.Backup(ctx, h.ClusterID, h.TaskID, h.RunID, target)
		close(done)
	}()
	...
	if err := h.service.Backup(context.Background(), h.ClusterID, h.TaskID, resumeRunID, target); err != nil {
