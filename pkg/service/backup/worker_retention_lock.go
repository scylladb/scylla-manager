...
func (w *worker) RetentionLock(egCtx context.Context, hosts []hostInfo, target Target) error {
	...
	for _, h := range hosts {
		eg.Go(func() error {
			if err := w.lockHostFiles(egCtx, h, cfg); err != nil {
				return errors.Wrapf(err, "lock host %s files", h.IP)
			}
			return nil
		})
	}
...
func (w *worker) lockSchemaFiles(ctx context.Context, hosts []hostInfo, cfg retentionLockConfig) error {
	...
	for _, hi := range hosts {
		if _, ok := doneLocations[hi.Location.StringWithoutDC()]; ok {
			continue
		}
		doneLocations[hi.Location.StringWithoutDC()] = struct{}{}
		w.Logger.Info(ctx, "Locking schema files", "location", hi.Location.StringWithoutDC(), "host", hi.IP)
		...
		remoteSchemaDir := hi.Location.RemotePath(path.Dir(cqlPath))

		if err := w.lockAndAwait(ctx, hi.IP, remoteSchemaDir, paths, cfg, "", ""); err != nil {
			return errors.Wrapf(err, "await retention lock with node %s", hi.IP)
		}
	}
	return nil
}
