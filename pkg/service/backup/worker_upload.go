...
func (w *worker) waitRestoreJob(ctx context.Context, pr *RestoreRunProgress) (err error) {
	defer func() {
		// Running stop procedure in a different context because original may be canceled
		stopCtx := context.Background()
	...
	for {
		...
		default:
			...
			case scyllaclient.JobSuccess:
				w.updateRestoreProgress(ctx, pr, job)
				return nil
			case scyllaclient.JobRunning:
				w.updateRestoreProgress(ctx, pr, job)
			case scyllaclient.JobNotFound:
...
func (w *worker) updateRestoreProgress(ctx context.Context, pr *RestoreRunProgress, job *scyllaclient.RcloneJobProgress) {
	pr.StartedAt = nil
	// Set StartedAt and CompletedAt based on Job
	if t := time.Time(job.StartedAt); !t.IsZero() {
		pr.StartedAt = &t
