...
func (w *worker) uploadHost(ctx context.Context, h hostInfo) error {
	...
	scyllaBackupAPISupport, err := w.supportsScyllaBackupAPI(ctx, h.IP, h.Location.Provider)
	if err != nil {
		return errors.Wrap(err, "check native backup API support")
	}
	if scyllaBackupAPISupport {
	...
	f := func(i int) (err error) {
		...
		if ok := w.useScyllaBackupAPI(ctx, d); scyllaBackupAPISupport && ok {
			w.Logger.Info(ctx, "Use Scylla backup API",
				"host", h.IP, "keyspace", d.Keyspace, "table", d.Table)
