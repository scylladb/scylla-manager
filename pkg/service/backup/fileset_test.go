...
func TestFilesSetFiles(t *testing.T) {
	fs := make(fileSet)
	...
	files := []struct {
		...
		{dir: "/a/b", files: []string{"c", "d", "e"}},
		{dir: "/z/o", files: []string{"o"}},
	}

	t.Run("has", func(t *testing.T) {
		for _, f := range files {
			fs.AddFiles(f.dir, f.files)
		}
		for _, f := range files {
			if !fs.HasAnyFiles(f.dir, f.files) {
				t.Errorf("fs.Has(%s) = false, expected true", f)
			}
		}
		if fs.Has("/a/b") {
	...
	t.Run("remove", func(t *testing.T) {
		d := files[0].dir
		f := files[0].files
		fs.RemoveFiles(d, f[0:1])
		if fs.HasAnyFiles(d, f[0:1]) {
			t.Errorf("Unexpected success")
		}
	})
