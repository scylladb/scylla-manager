...
1. Backup locations (buckets) with backed-up data created by [backup](https://manager.docs.scylladb.com/master/sctool/backup.md#sctool-backup) task.
2. Snapshot tag of backup that you want to restore.

To get a list of all available snapshot tags, use [sctool backup list](https://manager.docs.scylladb.com/master/sctool/backup.md#backup-list) command.
...
* `-c` - the [name or ID](https://manager.docs.scylladb.com/master/sctool/cluster.md#cluster-add) of the restore destination cluster
...
2. Restore schema using [sctool restore](https://manager.docs.scylladb.com/master/sctool/restore.md#sctool-restore).
   ```none
   sctool restore -c prod-cluster -L s3:test-bucket -T snapshot_tag --restore-schema
   restore/3208ff15-6e8f-48b2-875c-d3c73f545410
   ```
3. Ensure that restoring schema has finished using [sctool progress](https://manager.docs.scylladb.com/master/sctool/progress.md#task-progress).
   ```none
...
4. Ensure that all prerequisites of restoring the content of the tables are met.
   ...
   The easiest way to achieve that is to restore schema with [sctool restore](https://manager.docs.scylladb.com/master/restore/restore-schema.md) (so that’s what has already been done in this example).
...
5. Restore the content of the tables using [sctool restore](https://manager.docs.scylladb.com/master/sctool/restore.md#sctool-restore).
   ```none
   ...
   ```
6. Ensure that restoring the content of the tables has finished using [sctool progress](https://manager.docs.scylladb.com/master/sctool/progress.md#task-progress).
   ```none
