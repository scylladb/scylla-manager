...
The user must have list, read, write and delete privileges to the bucket, for specifics consult [the sample AWS IAM policy](https://manager.docs.scylladb.com/stable/backup/setup-amazon-s3.md#aws-iam-policy).
...
1. Create a user by providing access and secret keys.
   ...
   ```
2. Copy [the sample AWS IAM policy](https://manager.docs.scylladb.com/stable/backup/setup-amazon-s3.md#aws-iam-policy) as `user-policy.json`.
