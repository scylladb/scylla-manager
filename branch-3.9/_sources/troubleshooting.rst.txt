...
#. :ref:`Add ScyllaDB Manager Agent <install-agent>` to the new node. Use the **same** authentication token as you did for the other nodes in this cluster. Do not generate a new token.

#. Confirm the node / datacenter was added by checking its :ref:`sctool status <status>`. From the node running ScyllaDB Manager server run the ``sctool status`` command, using the name of the managed cluster.
