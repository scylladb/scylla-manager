...
func NewCommunicator(host string, config Config, dial DialContextFunc, logger log.Logger) *Communicator {
	return &Communicator{
		host:   host,
		config: config,
		dial:   dial,
...
func (c *Communicator) Connect(ctx context.Context) (err error) {
	c.logger.Info(ctx, "Connecting to remote host...")
	...
	c.logger.Info(ctx, "Connected!")

	if c.config.KeepaliveEnabled() {
		c.logger.Debug(ctx, "Starting ssh KeepAlives")
...
func (c *Communicator) Start(ctx context.Context, cmd *Cmd) error {
	...
	c.logger.Debug(ctx, "Starting remote command", "cmd", cmd.Command)
	...
	go func() {
		...
		if err != nil {
			c.logger.Debug(ctx, "Remote command exited with error", "cmd", cmd.Command, "status", exitStatus, "error", err)
		} else {
			c.logger.Debug(ctx, "Remote command exited", "cmd", cmd.Command, "status", exitStatus)
		}
...
func (c *Communicator) newSession(ctx context.Context) (session *ssh.Session, err error) {
	c.logger.Debug(ctx, "Opening new ssh session")
	if c.client == nil {
	...
	if err != nil {
		c.logger.Info(ctx, "ssh session open error", "error", err)
...
func (c *Communicator) Upload(ctx context.Context, path string, perm os.FileMode, src io.Reader) error {
	...
	c.logger.Debug(ctx, "Uploading file", "path", path, "perm", perm.Perm())
	...
	if err != nil {
		c.logger.Debug(ctx, "Uploading file ended with error", "path", path, "perm", perm.Perm(), "error", err)
	} else {
		c.logger.Debug(ctx, "Uploading file ended", "path", path, "perm", perm.Perm())
	}
...
func (c *Communicator) scpSession(ctx context.Context, scpCommand string, f func(io.Writer, *bufio.Reader) error) error {
	...
	if err != nil {
		if exitErr, ok := err.(*ssh.ExitError); ok {
			...
			// the exit status
			c.logger.Error(ctx, "scp error", "error", exitErr)
